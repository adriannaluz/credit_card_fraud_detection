...
# xgb_model = xgb.XGBClassifier(objective="binary:logistic", eval_metric='aucpr', scale_pos_weight=class_ratio, random_state=0)
xgb_model = xgb.XGBClassifier(scale_pos_weight=class_ratio, random_state=0)
LR_model = LogisticRegression(random_state=0) # LR_model = LogisticRegression(penalty='l2', C=10)

xgb_pipeline = make_pipeline(StandardScaler(), xgb_ model)
